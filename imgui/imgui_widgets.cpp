...
bool ImGui::CustomSlider(const char* label, float* value, float minValue, float maxValue, const ImVec2& size)
{
    ImGui::Text(label);
    bool value_changed = false;
    ...
    float grabX = pos.x + normalizedValue * sliderWidth;
    float grabRadius = 10.0f; 

    drawList->AddRectFilled(ImVec2(pos.x, pos.y + sliderHeight / 2), ImVec2(pos.x + sliderWidth, pos.y + sliderHeight / 2 + sliderHeight), IM_COL32(100, 100, 100, 255), sliderHeight / 2);

    drawList->AddCircleFilled(ImVec2(grabX, pos.y + sliderHeight / 2), grabRadius, IM_COL32(255, 165, 0, 255)); 

    ImGui::InvisibleButton(label, size);
    if (ImGui::IsItemActive() && ImGui::IsMouseDragging(ImGuiMouseButton_Left))
    {
        float mouseX = ImGui::GetMousePos().x;
        normalizedValue = (mouseX - pos.x) / sliderWidth;
        normalizedValue = normalizedValue < 0.0f ? 0.0f : (normalizedValue > 1.0f ? 1.0f : normalizedValue);
        float new_value = minValue + normalizedValue * (maxValue - minValue);
        if (new_value != *value) {
            *value = new_value;
            value_changed = true;
        }
